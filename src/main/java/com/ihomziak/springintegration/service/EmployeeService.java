...
public class EmployeeService {
    ...
    @Router(inputChannel = "social-account-channel")
    public String getSocialAccount(Message<Employee> message) {
        String department = message.getPayload().getEmployeeSocialAccount();

        return switch (department) {
            case "Whatsapp" -> "whatsapp-channel";
            case "Instagram" -> "instagram-channel";
            case "Twitter" -> "twitter-channel";
            case "Facebook" -> "facebook-channel";
            default -> "default-channel"; // Optional: handle unknown departments
        };
    }

    @ServiceActivator(inputChannel = "whatsapp-channel")
    public void getWhatsappAccount(Message<Employee> employee) {
        Message<String> marketing = MessageBuilder.withPayload("Whatsapp Account").build();
        log.info("Received message from: {}", marketing.getPayload());
    ...
    public void getInstagramAccount(Message<Employee> employee) {
        Message<String> marketing = MessageBuilder.withPayload("Instagram Account").build();
        log.info("Received message from: {}", marketing.getPayload());
    ...
    public void getTwitterAccount(Message<Employee> employee) {
        Message<String> marketing = MessageBuilder.withPayload("Twitter Account").build();
        log.info("Received message from: {}", marketing.getPayload());
    ...
    public void getFacebookAccount(Message<Employee> employee) {
        Message<String> marketing = MessageBuilder.withPayload("Facebook Account").build();
        log.info("Received message from: {}", marketing.getPayload());
